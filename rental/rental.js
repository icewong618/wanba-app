(() => {
  ...
  const exitRental = () => window.LeshenghuoModuleBridge.back('/');
  ...
  const stripePaymentMethodText = method => {
    const value=String(method||'');
    const test=value.startsWith('stripe_test');
    const type=value.replace(/^stripe(?:_test)?_?/, '') || 'card';
    const names={apple_pay:'Apple Pay',google_pay:'Google Pay',link:'Link',card:'信用卡'};
    const label=names[type]||'Stripe 安全付款';
    return test ? `Stripe 测试付款 · ${label}（未产生真实扣款）` : value.startsWith('stripe') ? `Stripe 付款 · ${label}` : '';
  };
  const bookingPaymentText = booking => stripePaymentMethodText(booking?.payment_method)
    || ({pending:'待付款',processing:'付款处理中',paid:'已付款，等待商家确认',refunded:'已退款',failed:'付款未完成'})[booking?.payment_status]
    || booking?.payment_status || '待付款';
  function renderList(){
  ...
  function renderBookingDetail(){
    ...
    const paymentLabel = bookingPaymentText(booking);
  ...
  function renderSuccess(booking){ const test=isTestPayment()||String(booking?.payment_method||'').startsWith('stripe_test'); app.innerHTML=`${top(test?'测试付款完成':'预约已提交')}<div class="success"><i>✓</i><h1>${test?'测试付款完成':'预约已提交'}</h1><b>${esc(booking.booking_code||'')}</b><p>${test?'这是 Stripe 测试环境的模拟付款，未发生真实扣款，也不会产生真实收入。':'商家将确认你的预约和付款安排。'}</p><section class="price-card"><div><span>付款方式</span><b>${esc(bookingPaymentText(booking))}</b></div><div><span>车辆租金</span><b>${money(booking.base_amount)}</b></div>${Number(booking.addon_amount||0)?`<div><span>附加服务</span><b>${money(booking.addon_amount)}</b></div>`:''}<div class="total"><span>预估总额</span><b>${money(booking.total_amount)}</b></div></section><div class="action-grid"><button class="secondary" onclick="Rental.bookings()">查看我的预约</button><button class="primary" onclick="Rental.list()">返回车辆列表</button></div></div>`; }
  ...
  const stripeError = message => { const target=document.getElementById('stripePaymentError'); if(target){ target.hidden=false; target.textContent=message; } };
  async function createBooking(){
    ...
    if(!response.ok){ const message=String(data?.message||''); throw new Error(message.includes('unavailable')?'该时段刚被其他预约占用，请重新选择时间。':message.includes('cannot')?'这笔预约目前不能修改，请联系商家协助处理。':'预约提交失败，请稍后重试。'); }
    return data;
  }
  async function submit(){
  ...
  window.Rental={close,back,refreshList,select,quote,toggleAddon,contact,review,pickPayment:id=>{state.paymentMethod=id;renderPayment();},submit,confirmStripePayment,modify,cancel,bookings:async()=>{await loadBookings();state.screen='bookings';renderBookings();},booking:openBooking,cancelBooking,editBooking,setFilter:(key,value)=>{state.filters[key]=value;renderList();},clearFilters:()=>{state.filters={seats:'',fuel:'',type:''};renderList();},list:()=>{state.screen='list';state.selected=null;state.quote=null;state.editingBookingId='';renderList();},help:()=>alert(`请联系 ${state.merchant?.business_name||'商家'}：${state.merchant?.phone||'商家暂未填写电话'}`),faq:()=>alert('常见问题：预约提交后由商家确认；请在取车前携带有效驾照与预约信息。')};
  load();
})();
