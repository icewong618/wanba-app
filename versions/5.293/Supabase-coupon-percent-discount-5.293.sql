-- 乐生活 v5.293：优惠券支持“优惠 10% / 立减 10%”等百分比抵扣。
...
begin
  ...
  for claim_id in select value::bigint from jsonb_array_elements_text(coalesce(p_coupon_claim_ids,'[]'::jsonb)) value loop
    ...
    if jsonb_typeof(coalesce(coupon_rule->'weekdays','[]'::jsonb))='array' and jsonb_array_length(coalesce(coupon_rule->'weekdays','[]'::jsonb))>0
      and not exists(select 1 from jsonb_array_elements_text(coupon_rule->'weekdays') day_value where day_value=la_weekday::text) then raise exception 'coupon_invalid_weekday'; end if;
    ...
    if jsonb_typeof(permitted_methods)='array' and jsonb_array_length(permitted_methods)>0
      and not exists(select 1 from jsonb_array_elements_text(permitted_methods) method_value where (p_payment_method='online' and method_value in ('online','card')) or (p_payment_method<>'online' and method_value=p_payment_method)) then raise exception 'coupon_invalid_payment_method'; end if;
    ...
    if price_match is not null then
      ...
      if price_match is not null and price_match[1]::numeric > 0 and price_match[1]::numeric < 10 then
        ...
        price_match := regexp_match(coupon_text, '(优惠|立减|折扣|减免)?[[:space:]]*([0-9]+([.][0-9]+)?)[[:space:]]*%');
        if price_match is not null and price_match[2]::numeric > 0 and price_match[2]::numeric <= 100 then
          discount_value := discount_value + subtotal_value * price_match[2]::numeric / 100;
        end if;
      end if;
    end if;
  end loop;
  ...
  update public.merchant_orders set payment_status='paid', paid_at=now(), paid_method=p_payment_method,
      status=case when remaining_count=0 then 'completed' else status end,
      completed_at=case when remaining_count=0 then now() else completed_at end, updated_at=now() where id=p_order_id;
  ...
  values(p_order_id,order_row.merchant_user_id,order_row.user_id,p_payment_method,subtotal_value,discount_value,tip_value,subtotal_value-discount_value+tip_value,coalesce(p_coupon_claim_ids,'[]'::jsonb)) returning * into bill_row;
  return bill_row;
end;
$$;
