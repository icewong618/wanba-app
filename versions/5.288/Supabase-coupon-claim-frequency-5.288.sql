-- 乐生活 v5.288：优惠券领取频率由商家配置，并由数据库强制执行。
...
begin
  ...
  if claim_mode not in ('unlimited','once','daily','weekly','monthly','interval') then claim_mode := 'once'; end if;
  ...
  if claim_mode='daily' and latest_claim is not null
    and (latest_claim at time zone 'America/Los_Angeles')::date = (now() at time zone 'America/Los_Angeles')::date then
    raise exception 'coupon_claim_daily_limit';
  end if;
  if claim_mode='weekly' and latest_claim is not null
    and date_trunc('week', latest_claim at time zone 'America/Los_Angeles') = date_trunc('week', now() at time zone 'America/Los_Angeles') then
    raise exception 'coupon_claim_weekly_limit';
  end if;
  if claim_mode='monthly' and latest_claim is not null
    and date_trunc('month', latest_claim at time zone 'America/Los_Angeles') = date_trunc('month', now() at time zone 'America/Los_Angeles') then
    raise exception 'coupon_claim_monthly_limit';
  end if;
  if claim_mode='interval' and latest_claim is not null
    and latest_claim > now() - make_interval(days => interval_days) then
    raise exception 'coupon_claim_interval_limit';
  end if;
...
revoke all on function public.claim_merchant_coupon(uuid,text) from public, anon;
