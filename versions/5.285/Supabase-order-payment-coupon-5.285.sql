-- 乐生活 v5.285：付款与出餐分离；收银核销优惠券时由数据库复算订单金额。
...
begin
  ...
  for claim_id in select value::bigint from jsonb_array_elements_text(coalesce(p_coupon_claim_ids,'[]'::jsonb)) value loop
    ...
    if jsonb_typeof(permitted_methods)='array' and jsonb_array_length(permitted_methods)>0
      and not exists(select 1 from jsonb_array_elements_text(permitted_methods) method_value where method_value = case when p_payment_method='online' then 'card' else p_payment_method end) then
      raise exception 'coupon_invalid_payment_method';
    end if;

    coupon_text := coalesce(coupon_rule->>'title','') || ' ' || coalesce(coupon_rule->>'description','');
    price_match := regexp_match(coupon_text, '满\\s*([0-9]+(?:\\.[0-9]+)?)\\s*(?:元|美元|\\$)?\\s*减\\s*([0-9]+(?:\\.[0-9]+)?)');
    if price_match is not null then
      if subtotal_value >= price_match[1]::numeric then discount_value := discount_value + price_match[2]::numeric; end if;
    else
      price_match := regexp_match(coupon_text, '([0-9]+(?:\\.[0-9]+)?)\\s*折');
